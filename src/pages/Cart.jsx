...
import { clearCartAction, deleteCartItemAction, plusCartItemAction, minusCartItemAction } from "../redux/actions/cart"
...
function Cart() {
  ...
  const arrCartPizza = Object.keys(items).map((key) => items[key].arrPizza[0]); //массив в котором хронятся все выбранные виды пицц
  ...
  const handleDeleteItemCart = (obj) => {
    if (window.confirm("Вы действительно хотите удалить товар?")) {
      dispatch(deleteCartItemAction(obj))
    }
  }


  const handlePlusItemCart = (obj) => {
    dispatch(plusCartItemAction(obj))
  }

  const handleMinusItemCart = (obj) => {
    dispatch(minusCartItemAction(obj))
  }
      ...
      {totalCount !== 0 ?
        (<div className="cart">
          ...
          <div className="content__items">
            {arrCartPizza.map((obj) =>
              <CartItem
                key={`${obj.id}_${obj.type}_${obj.size}`}
                id={obj.id}
                name={obj.name}
                type={obj.type}
                size={obj.size}
                imageUrl={obj.imageUrl}
                totalCount={items[`${obj.id}_${obj.type}_${obj.size}`].arrPizza.length}
                totalPrice={items[`${obj.id}_${obj.type}_${obj.size}`].totalPrice}
                onDeleteItemCart={handleDeleteItemCart}
                onPlusItemCart={handlePlusItemCart}
                onMinusItemCart={handleMinusItemCart}
              />
            )}
          </div>
